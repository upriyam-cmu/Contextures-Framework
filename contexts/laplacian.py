...
def laplacian_kernel(x, x_prime, gamma):
    """
    Computes the laplacian kernel between two sets of data points.
    Args:
    ...
    """
    diff = torch.abs(x.unsqueeze(1) - x_prime.unsqueeze(0))
...
class Laplacian(Contexts):
    ...
    def fit(self, dataset: DataFrame) -> None:
        ...
        self.X_train = X
        kx = laplacian_kernel(self.X_train, self.X_train, self.gamma)
    ...
    def _sample(self, x: Tensor) -> Tensor:
        ...
        # Repeat for num_context_samples
